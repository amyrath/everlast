...
// Importe toutes les icônes nommées sous l'objet LucideIcons
import * as LucideIcons from 'lucide-react';

// --- Configuration des icônes pour les spécifications ---
// Cette fonction retourne toujours l'icône de coche simple ('Check'), conformément à la demande.
const getIconForSpec = () => {
    // Retourne toujours LucideIcons.Check (l'icône de coche SANS le cercle)
    return LucideIcons.Check; 
};


// --- BASE DE DONNÉES DES PRODUITS (Mise à jour des spécifications) ---

// Spécifications communes à TOUS les produits
const generalSpecs = [
    "Available with Mosquito Net Screen",
    "Wide selection of Frame Colours",
    "Optional Tinted Glass in multiple shades",
    // AJOUT GLOBAL DEMANDÉ : Réduction sonore/Isolation
...
const PRODUCTS_DB = {
    windows: {
        1: { 
            ...
            // Spécifications de Sliding Windows mises à jour
            specs: [
        ...
        3: {
            ...
            // Spécifications de Fixed Windows : Le 'sound reduction' est couvert par generalSpecs.
            specs: ["Absolute Air and Water Tightness", "Unrivaled Security", "Maximal Light Intake", "Low Maintenance", "Custom Shapes Available", ...generalSpecs],
        },
        4: {
    ...
    doors: {
        ...
        2: {
            ...
            specs: ["Dual Functionality (Tilt/Slide)", "uPVC 7-Chamber Profile", "Optimal Ventilation", "Superior Sound Insulation", "High Security Mechanism", ...generalSpecs],
        },
        3: {
...
const ProductDetailPage = ({ params }) => {
    ...
    return (
        <section className="py-12 md:pt-16 md:pb-24 bg-gray-100 text-gray-900 font-corps">
            <div className="container mx-auto px-4 max-w-5xl">
                
                {/* LIEN DE RETOUR À L'ACCUEIL */}
                <Link 
                    href="/#products" 
                    className="group flex items-center text-lg font-semibold text-gray-600 hover:text-red-700 transition duration-300 mb-8"
                >
                    <svg className="w-5 h-5 mr-2 inline-block align-text-bottom transition-transform group-hover:-translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                ...
                <div className="mb-12 rounded-2xl shadow-2xl overflow-hidden border-4 border-white transform hover:scale-[1.01] transition duration-500">
                    <Image 
                        src={product.image} 
                ...
                {/* DESCRIPTION (Bloc principal avec couleur de fond pour donner de l'entrain) */}
                <div className="p-8 bg-white rounded-2xl shadow-2xl border-t-8 border-blue-900 mb-12">
                    <h2 className="font-titre text-3xl font-bold text-blue-900 mb-4">Detailed Description</h2>
                    <p className="font-corps text-xl leading-relaxed text-gray-700 border-b pb-6 mb-6 border-gray-100">
                        {product.description}
                    </p>
                    <a href="/contact" className="mt-4 inline-block px-8 py-3 bg-red-700 text-white font-bold text-lg rounded-full shadow-lg transition duration-300 hover:bg-red-800 hover:shadow-xl transform hover:-translate-y-0.5">
                        Request a Custom Quote →
                    </a>
                </div>
                
                {/* SPÉCIFICATIONS (Bloc séparé avec accentuation visuelle) */}
                <div className="p-8 bg-blue-50 rounded-2xl shadow-xl">
                    <h2 className="font-titre text-3xl font-bold text-red-700 mb-6 border-b pb-3 border-blue-200">Key Specifications</h2>
                    <ul className="space-y-4 font-corps text-xl text-gray-800">
                        {product.specs.map((spec, index) => {
                            // Récupère le composant d'icône (toujours Check).
                            const IconComponent = getIconForSpec(); 
                            return (
                                <li key={index} className="flex items-start">
                                    {/* Icônes de coche simple en rouge pour attirer l'attention sur les points forts */}
                                    <IconComponent size={24} className="mr-3 text-red-700 flex-shrink-0 mt-1" />
                                    {spec}
                                </li>
                            );
                        })}
...
//       image: "/images/turn-door.jpg", 
//       description: "Robust and highly elegant, our Turn Doors function like traditional hinged doors, providing superior security and unmatched thermal performance. Ideal for main entrances or balcony access, they ensure a perfect seal and long-lasting operation.",
//       specs: ["High Security Mechanism", "Excellent Thermal Insulation", "Durable uPVC Hinges", "Main Entrance Compatible", "Reinforced Steel Frame", "Available in Single or Double"],
//     },
//   }
// };
